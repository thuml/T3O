...
def generate_latex_table(df, file_name='latex_table_big.tex'):
    with open(file_name, 'w') as f:
        ...
        for dataset in datasets:
            ...
            for i, pred_len in enumerate(prediction_lengths):
                ...
                if i==0:
                    row = f"\\multirow{{4}}{{*}}{{\\rotatebox{{90}}{{{dataset}}}}} & {pred_len} "
                else:
                    row = f"& {pred_len} "
                for model in ['Timer-UTSD', 'Timer-LOTSA', 'MOIRAI-small', 'MOIRAI-base', 'MOIRAI-large',
                    ...
                    mae_str = f"{mae:2.2f}"
                    if mse > 0:
                        mse_str = f"\\textbf{{\\textcolor{{blue}}{{{mse:2.2f}}}}}"
                    if mae > 0:
                        mae_str = f"\\textbf{{\\textcolor{{blue}}{{{mae:2.2f}}}}}"

                    row += f"& {mse_str} & {mae_str} "
                row += "\\\\\n"
                f.write(row)
            f.write('\\midrule\n') if dataset != datasets[-1] else f.write('\\bottomrule\n')
