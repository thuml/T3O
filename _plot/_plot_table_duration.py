...
def generate_latex_table(df_samples, df_params, df_duration, models, file_name='latex_table_duration.tex'):
    # \multicolumn{2}{c}{\textbf{Overhead}} & \textbf{Timer-UTSD} & \textbf{Moirai-base} & \textbf{Chronos-tiny} \\
    # \midrule
    # \multirow{3}{*}{Trials}
    ...
    with open(file_name, 'w') as f:
        ...
        # f.write('\\centering\n')
        # f.write('\\caption{Adaptation and Inference Overhead (per dataset)}\n')
        # f.write('\\begin{tabular}{cccccc}\n')
        # f.write('\\toprule\n')
        # f.write(
        #     '\\multicolumn{3}{c}{\\textbf{Models}} & \\textbf{Timer-UTSD} & \\textbf{MOIRAI-base} & \\textbf{CHRONOS-tiny} \\\\\n')
        # f.write('\\midrule\n')
        f.write(
            '\\multirow{6}{*}{\\shortstack{Adaptation Overhead \\\\ (per dataset)}} & \\multirow{3}{*}{\\shortstack{Number of \\\\ Operator Combinations \\\\ (samples=100)}}\n')
        ...
        f.write('& \\multirow{3}{*}{\\shortstack{Number of \\\\ Data Samples \\\\ (combination=100)}}\n')
