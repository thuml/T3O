    ...
    with open(file_name, 'w') as f:
        ...
        for dataset in datasets:
            ...
            for i, mode in enumerate(display_mode):
                ...
                # 新增两列计算MAE和MSE的Average：各dataset的 MSE和MAE的%Promotion的均值
                # if mode == '%Promotion':
                ...
                #     row += f"& {mse_str} & {mae_str} "
                # elif mode == 'Vanilla':
                #     mse = df_dataset['org_mse'].mean()
                #     mae = df_dataset['org_mae'].mean()
                #     mse_str = f"{mse:2.4f}"
                #     mae_str = f"{mae:2.4f}"
                #     row += f"& {mse_str} & {mae_str} "
                # elif mode == 'Our':
                #     mse = df_dataset['our_mse'].mean()
                #     mae = df_dataset['our_mae'].mean()
                #     mse_str = f"{mse:2.4f}"
                #     mae_str = f"{mae:2.4f}"
                #     row += f"& {mse_str} & {mae_str} "
                org_mse = df_dataset['org_mse'].mean()
                org_mae = df_dataset['org_mae'].mean()
                our_mse = df_dataset['our_mse'].mean()
                our_mae = df_dataset['our_mae'].mean()
                imp_mse = (org_mse - our_mse) / org_mse * 100
                imp_mae = (org_mae - our_mae) / org_mae * 100
                if mode == 'Vanilla':
                    mse, mae = org_mse, org_mae
                    mse_str, mae_str = f"{mse:2.4f}", f"{mae:2.4f}"
                elif mode == 'Our':
                    mse, mae = our_mse, our_mae
                    mse_str, mae_str = f"{mse:2.4f}", f"{mae:2.4f}"
                elif mode == '%Promotion':
                    mse, mae = imp_mse, imp_mae
                    mse_str, mae_str = f"\\textbf{{{mse:2.1f}\\%}}", f"\\textbf{{{mae:2.1f}\\%}}"
                row += f"& {mse_str} & {mae_str} "
        ...
        # 新增最后几行计算Average：各自model的 MSE和MAE的%Promotion的均值（需要计算原始值）
        f.write('\\midrule\n')
        dataset = 'Average'
        df_dataset = df
        for i, mode in enumerate(display_mode):
            ...
            if i == 0:
                ...
                row = f"\\multirow{{3}}{{*}}{{\\textbf{{{dataset}}}}} & {mode_str} "
            else:
                row = f"& {mode_str} "
            for model in ['Timer-UTSD', 'Timer-LOTSA', 'MOIRAI-small', 'MOIRAI-base', 'MOIRAI-large', 'Chronos-tiny']:
                # 取均值
                org_mse = df_dataset[df_dataset['model_name'] == model]['org_mse'].mean()
                org_mae = df_dataset[df_dataset['model_name'] == model]['org_mae'].mean()
                our_mse = df_dataset[df_dataset['model_name'] == model]['our_mse'].mean()
                our_mae = df_dataset[df_dataset['model_name'] == model]['our_mae'].mean()
                imp_mse = (org_mse - our_mse) / org_mse * 100
                imp_mae = (org_mae - our_mae) / org_mae * 100
